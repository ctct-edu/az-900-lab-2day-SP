...
5. **「ポートルールの作成」**から**「受信ポートルール」** を選択して以下の設定を入力します。完了したら、**「追加」** をクリックします。 
...
8. **「ネットワークセキュリティグループaz900-07-nsg1 (ネットワーク インターフェイスにアタッチ済み: az900-07-vmxxxx)** の右側にある **「ポートルールの作成」** から **「送信ポートルール」** を選択してより優先度の高い、インターネットへのトラフィックを拒否する新しい送信セキュリティ規則を構成します。以下の内容を入力、完了したら **「追加」** をクリックします。 
