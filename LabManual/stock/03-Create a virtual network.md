...
2. ダウンロードしたRDPファイルを使用して、接続を行います。

3. 「**Windows セキュリティ**」 ウィンドウで、仮想マシンの作成時に設定した管理者ユーザ名「**Student**」とパスワード「 **Pa55w.rd1234**」 を使用してサインインします。 

4. サインイン プロセス中に証明書の警告が表示されることがあります。「**はい**」をクリックして、仮想マシンに接続します。

5. Azure ポータルの「**Virtual Machines**」 ブレードに戻って**az900-04-vm2** をクリックし、同様にRDP接続を実施します。

6. 新しく作成された**両方**の仮想マシン上で、 **「スタート」メニュー> 「歯車マーク(Settings)」 > 「Network ＆ Internet」　> 「Windows Firewall」** を開いて、Private Network (プライベートネットワーク)をクリックします。

    ![04-01](./images/04-01.png)

7. Windows Defender Firewall のスライダーを無効化 (Off) にします。

    ![04-02](./images/04-02.png)

8. **「Windows Firewall」** に戻り、 Public network も同様に、Windows Defender Firewall のスライダーを無効化 (Off) にします。

    **※az900-04-vm1、az900-04-vm2の両方の仮想マシンでWindows Defender Firewallを無効化してください。**

9. az900-04-vm1 のRDP接続先のタスクバーの検索ボックスから「**PowerShell**」 を検索します。検索結果より、**Windows PowerShell**を右クリックして「**管理者として実行(Run as administrator)**」します。

10. PowerShell で、次のように入力して az900-04-vm2 に ping を実行してみます。

    ```PowerShell
    ping az900-04-vm2
    ```

11. az900-04-vm1 から az900-04-vm2 への ping が成功するはずです。
